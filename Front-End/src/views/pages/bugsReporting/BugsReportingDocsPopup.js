...
import BugsReportingDocsPdfViewer from './BugsReportingDocsPdfViewer';
...
const BugsReportingDocsPopup = ({ isVisible, onClose, rowData }) => {
  ...
  useEffect(() => {
    if (isVisible && rowData) {
      const newDataSource = new CustomStore({
        key: 'id',
        load: () => loadBugReportingDocs(),
        remove: (key) => deleteBugReportingDocsNDetails(key)
      });
  ...
  const handleFileUpload = async (e) => {
    ...
    formData.append('bugsReportingId', rowData?.id);

    try {
      setUploading(true);

      await axiosInstance.post('/bugsReporting-docs/upload', formData, {
  ...
  const loadBugReportingDocs = async () => {
    if (!isVisible || !rowData?.id) {
      return [];
    }

    try {
      const response = await axiosInstance.get(`/bugsReporting-docs/getBugReportDocDetailBy/${rowData.id}`);
      return response.data || [];
    } catch (error) {
      notify(`Error loading Bug Reporting Docs Data: ${error.message}`, 'error', 3000);
      return [];
    }
  };

  const deleteBugReportingDocsNDetails = async (key) => {
    try {
      await axiosInstance.delete(`/bugsReporting-docs/delete/${key}`);
      notify('Bug Reporting document deleted successfully', 'success', 2000);
    } catch (error) {
...
export default BugsReportingDocsPopup;